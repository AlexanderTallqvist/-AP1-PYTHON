...
# Uppg. 1 (Antar att svart vid situation där m = n, är m)
# Remove comment on line 20 to run
...
# Uppg. 2
# Remove comment on line 45 to run
...
# Uppg. 3C
# Remove comment on line 286 to run

# Spelet väljer turvis en slumpmässig pjäs för vita och svarta,
# och försöker sedan flytta  pjäsen till bästa möjliga position.
# Vid situationer där pjäsen inte kan flyttas, väljs det en ny slumpmässig pjäs.

# Spelet slutar då antigen vita eller svarta når den andra ändan av spelbrädet,
# eller då alla pjäser av en färg blivit uppätna.
...
def take_turn(iterator):
    ...
    # It is blacks turn to move. Make sure the color has any pieces left to move
    if iterator % 2 == 0:
        if len(blacks) > 0:
    ...
    # It is whites turn to move. Make sure the color has any pieces left to move
    else:
        if len(whites) > 0:
...
def move_a_piece(direction, color, enemy_color, color_string, enemy_string, enemy_letter, win_condition):
    ...
    # Pick a random piece from the color array
...
# Uppg. 4
# Remove comment on line 337 to run
...
def play_a_game(cards, iterator):
...
for i in range(rounds):
  suffeled_cards = cardshuffle()
  outcome = play_a_game(suffeled_cards, 1)
...
#print "We won the game", win, "times.", "\nWe lost the game", loss, "times."


# Uppg. 5

# myshuffle byter plats på två stycken slumpmässiga kort i kortleken 52 gånger.
# Eftersom att en välblandad kortlek kan befinna sig i 52! olika positioner,
# så kan det anses vara onödigt att blanda kortleken mera än en gång, eftersom att
# chanses att kortleken någon gång skulle befinna sig i en ordrning som tidigare redan existerat, är närmare 0.
# (1/8065817517094390000000000000000000000000000000000000000000000000000)
